...
| `libros` | titulo, autor, genero, formato, estado_lectura, impacto_emocional, cita_memorable, calificacion, paginas_leidas, numero_paginas, **fecha_inicio**, **fecha_fin**, portada_url, notas (JSON), **created_at**, **origen** (`usuario`\|`catalogo`) |
...
- ✅ **Catálogo de libros gratis (2026-06-20)**: página **Explorar** (`/catalogo`) que busca en **Project Gutenberg** vía **Gutendex** (API JSON sin clave, filtro por idioma — español por defecto, solo libros con EPUB). "Añadir" descarga el EPUB al servidor (`/uploads/catalogo/`, compartido entre usuarios, dedup por id) y lo mete en la biblioteca con `origen='catalogo'` (no gasta cuota ni cuenta para el límite del plan gratis). Se lee con el lector normal. Backend `routes/catalog.js`, página `pages/Catalogo.jsx`, enlace en el menú (Explorar) y **CTA en la biblioteca vacía**. Soluciona el "app vacía al entrar" y evita problemas de derechos de autor (todo dominio público). *(Futuro: añadir Standard Ebooks por su mejor maquetación.)*
