...
// PRIVACIDAD: los libros que sube cada usuario son PRIVADOS y no compartibles. Bloqueamos el
// acceso directo a /uploads/books/* — solo se sirven a su dueño vía enlace firmado temporal
// (GET /api/books/file/:token). Esta línea va ANTES del static para tener prioridad.
app.use('/uploads/books', (req, res) => {
    res.status(403).json({ message: 'Acceso directo no permitido. Abre el libro desde tu cuenta.' });
});

// Resto de archivos subidos (audio de podcasts, portadas) servidos estáticamente.
